...
async def start(message, state):
    await bot.send_message(message.chat.id, "Привет, я MegaWeatherBot!\nЧтобы узнать погоду,"
                                            " напиши где находишься или подключи геолокацию", reply_markup=keyboard_location_inline)
...
async def start_i1(callback_query: types.CallbackQuery, state):
    await bot.answer_callback_query(callback_query.id)
    await bot.send_message(callback_query.from_user.id, 'Напиши название любого населенного пункта и я скажу тебе погоду там!')
